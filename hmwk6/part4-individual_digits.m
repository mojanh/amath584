...
% filter data to single digit
digit_filter = 9;
indices = find(train_labels==digit_filter);
train_labels(train_labels ~= digit_filter) = 0;
train_labels(train_labels == digit_filter) = 1;
...
[model_1,~,~,~] = cross_validate(5,0.01,"lasso",train_labels_vec,train_labels,double(train_images),test_labels,test_labels_vec,double(test_images'));

% check for important pixels by displaying X as pcolor
figure(1);
model_1(abs(model_1)<0.00005) = 0;
pcolor(reshape(model_1,28,28));ax = gca;ylabel(ax,'pixels');xlabel(ax,'pixels');
...
function [model,predictions,error,error_count] = cross_validate(k,parameter,model_type,train_labels_vec,train_labels,train_images,test_labels,test_labels_vec,test_images)
    ...
    for i=1:k
        ...
        single_fold_labels = double(train_labels(lower_bound:upper_bound,:));
        ...
        elseif model_type=="lasso"
            ...
            models(:,:,i) = lasso(single_fold_images,single_fold_labels,'Lambda',parameter); 
