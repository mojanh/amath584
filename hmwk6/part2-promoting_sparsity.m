...
% display loads for varying labmda values for ridge regression 
figure(1);
lambdas = [0 0.1 0.5];
for m=1:3
	[model_2,~,~,~] = cross_validate(5,lambdas(m),"ridge",train_labels_vec,train_labels,double(train_images),test_labels,test_labels_vec,double(test_images'));
    single_column = model_2(:,3);
	single_column(abs(single_column)<0.001) = [];
